...
def get_fundamental_data(ticker: str) -> dict:
    ...
    try:
        ...
        logger.warning(f"Lỗi lấy FA Yahoo Finance cho {ticker_upper}: {e}")
...
def get_macro_flow() -> str:
    ...
    try:
        ...
        logger.warning(f"Lỗi lấy dữ liệu Vĩ mô VNINDEX: {e}")
...
def is_ticker_in_news(ticker: str, text: str) -> bool:
    ...
    loại trừ triệt để các trường hợp False Positive phổ biến:
    - Mã 'VND': Tránh nhầm với đơn vị tiền tệ (ví dụ: '500 tỷ VND', '1.000 VND', 'USD/VND')
    - Mã 'BID': Tránh nhầm với thuật ngữ giao dịch 'giá bid' / 'bid-ask'
    """
    if not text:
        return False
    t = ticker.upper()
    if t == 'VND':
        # Loại bỏ các cụm tiền tệ trước khi xét ticker
        cleaned = re.sub(r'(\d+[\d\.,]*\s*(?:tỷ|triệu|nghìn|ngàn)?|\btỷ|\btriệu|\bnghìn|\bngàn|\bđồng|usd/)\s*VND\b', '', text, flags=re.IGNORECASE)
        return bool(re.search(r'\bVND\b', cleaned, flags=re.IGNORECASE))
    elif t == 'BID':
        cleaned = re.sub(r'\b(?:giá\s+bid|bid/ask|bid-ask)\b', '', text, flags=re.IGNORECASE)
        return bool(re.search(r'\bBID\b', cleaned, flags=re.IGNORECASE))
...
def get_latest_news(ticker: str) -> str:
    ...
    Sử dụng Regex chuyên biệt để lọc chính xác ticker, tránh False Positive (VND, BID, PAN...).
    """
    ticker_upper = ticker.upper()
    news_texts = []
    headers = {
    ...
    if not news_texts:
        try:
            ...
            logger.warning(f"Lỗi fallback tin tức CafeF: {e}")
